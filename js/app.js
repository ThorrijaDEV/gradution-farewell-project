...
<p>Y es que eso eres tú, Vanesa. La profesora que hace que todo el mundo se sienta válido, importante. Que hace que aprender Lengua no sea memorizar reglas aburridas, sino disfrutar de las palabras, de su poder, de lo que pueden hacer. Que hace un simple "¿me ayudas con esto?" se convierta en el mejor momento del día de alguien.</p>
...
function renderCards() {
    ...
    teachers.forEach((teacher, index) => {
        ...
        card.innerHTML = `
            ...
            <p class="card-quote">${teacher.quote}</p>
            <button class="card-btn" data-id="${teacher.id}">Leer carta</button>
        `;
...
function startExperience() {
    ...
    // Desactivamos el mute automáticamente al empezar para reproducir música
    isMuted = false;
    if (els.muteBtn) {
    ...
    // Transición cinemática e inicio libre de scroll
    els.introScreen.style.transition = 'opacity 0.8s ease-in-out, transform 0.8s ease-in-out';
    els.introScreen.style.transform = 'scale(0.95)';
    els.introScreen.style.opacity = '0';

    setTimeout(() => {
        els.introScreen.classList.remove('active');
        els.introScreen.style.display = 'none'; // ¡Básico para liberar el scroll!
...
// ============================================
// RETRO 8-BIT MUSIC (Web Audio API)
// ============================================
